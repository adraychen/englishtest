...
def get_pure_response(student_text: str, history: list, topic_name: str, student_level: int = 5) -> str:
    ...
    desc = "Topic: " + topic_name + "\n"
    desc += "Conversation so far:\n" + history_str + "\n"
    desc += "Friend just said: " + student_text + "\n\n"
    desc += "Reply naturally as Alex — casual, genuine, varied length."

    task = Task(
        description=desc,
        expected_output="A natural conversational reply adapted to the student level.",
        agent=agent,
    )
