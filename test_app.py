...
# ── Pure conversation agent (no coaching) ────────────────────────────────────
def get_pure_response(student_text: str, history: list, topic_name: str, student_level: int = 5) -> str:
    """Returns a natural conversational reply adapted to student level, zero coaching."""
    from crewai import Agent, Task, Crew, Process

    history_str = ""
    for msg in history[-8:]:
        role = "Friend" if msg["role"] == "student" else "Alex"
    ...
    if student_level <= 3:
        ...
        - Examples of your responses: "Oh nice! Where did you go?"
          "That sounds fun! What did you eat?" "Ha, same here! Do you go often?"
        """
        ...
        - Examples of your responses: "Oh that sounds amazing, I've always wanted
          to do that! Did you enjoy it?" "Ha same, I do that all the time.
          What's your favourite?" "Wait really? How was it?"
        """
    else:
        level_style = """
        ...
        - Examples of your responses: "Oh nice!", "Ha same!", "Wait really?",
          "That's so good!", "No way, I was just there last week!",
          "Oh I know exactly what you mean, it's such a vibe.",
          "Honestly same — what made you want to try it?"
        """
    ...
    return Agent(
        ...
        hiking, and trying new restaurants. You are texting a friend and having a
        casual chat.
        ...
        - Short reactions are great: "Oh nice!", "Ha same!", "Wait really?",
          "That's so good!", "No way!", "Seriously?"

        Bad question examples — never do this:
        - "Did you end up buying a bottle of your favourite from the tasting to take home?"
        - "What was your favourite part of the cherry blossom experience, was it the scenery, the atmosphere, or something else?"

        Good question examples — keep it like this:
    ...
    agent = get_pure_response(student_text, history, topic_name, student_level)

    task = Task(
        description=(
            "Topic: " + topic_name + "\n"
            "Conversation so far:\n" + history_str + "\n"
            "Friend just said: " + student_text + "\n\n"
            "Reply naturally as Alex — casual, genuine, varied length."
        ),
        expected_output="A natural conversational reply adapted to the student's level.",
        agent=agent,
    )
...
if "chat_mode"      not in st.session_state: st.session_state.chat_mode      = "pure"  # "pure" or "coached"
...
with col_m1:
    if st.button(
        "🗣️ Pure chat mode" if st.session_state.chat_mode == "pure" else "💬 Coached mode",
        use_container_width=True,
        type="primary" if st.session_state.chat_mode == "pure" else "secondary"
    ):
        pass  # label only — toggle below
with col_m2:
    if st.button(
        "Switch to coached mode →" if st.session_state.chat_mode == "pure" else "Switch to pure chat →",
        use_container_width=True
    ):
        new_mode = "coached" if st.session_state.chat_mode == "pure" else "pure"
...
mode_label = "🗣️ Pure conversation — no coaching" if st.session_state.chat_mode == "pure" else "💬 Coached conversation"
...
def handle_answer(user_text: str):
    ...
    with st.spinner("Thinking..."):
        if st.session_state.chat_mode == "pure":
            # Pure conversation — no coaching, no format, just natural chat
            reply = get_pure_response(
                user_text,
                st.session_state.history,
                current_topic["name"],
            ...
            next_audio = make_audio_b64(reply)
        else:
            # Coached mode — comment + suggestion + question
            ...
            next_audio = make_audio_b64(question if not is_last else comment)
    ...
    if is_last:
        ...
        st.session_state.pending_audio = next_audio
